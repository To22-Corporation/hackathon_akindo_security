...
「web3 に興味を持ち始めたが、Scam 被害が多くて怖い」
というユーザーのセキュリティの悩みを解決するサービス

### 【サービスの特徴】

・トークンのコントラクトアドレスを入力するだけで、リスクを簡単に分析
・リスクの判断根拠を透明化し、利用者のリテラシー向上にも貢献
・Chrome 拡張機能による自動検知によりスムーズなUX を実現

### 【Point】

● Technicality  
スキャム被害も増える中、リスク検知をいかに手軽に、無意識にできるかが非常に重要であり、私たちはこの点を解決  

● Originality  
情報がオープンである web3 の利点を最大限活かし、コントラクトに関わる全ての情報を使いリスクを判断  

● Practicality  
現状、コントラクトの潜在的な危険性の大小、及びその原因解析が可能  

● Usability  
拡張機能を使うことでリスクを自動で察知してくれる UX まで実現  
...
- また、今後色々なトークンや dApps が増え、ユーザが増えるほどこの問題は大きくなっていく。
...
# Appendix
トークン保有者集中度とScamトークンの関係性
今回のプロジェクトで作成したScamを確認する一つの指標としてトークン保有者の集中度合いを確認します。

これはScamトークンのほうがトークンが特定ユーザーに集中する傾向が見られたからです。
Scamプロジェクトとそうでないプロジェクトが発行するトークンのユーザー保有率の分布を以下のグラフでプロットされています。

<img width="825" alt="image" src="https://user-images.githubusercontent.com/41190754/200159322-016bbc69-bb41-4f54-85a8-262df8cdf6d5.png">

Scamプロジェクトのトークンのほうが上位ユーザーにトークンが集中していることがわかります。
